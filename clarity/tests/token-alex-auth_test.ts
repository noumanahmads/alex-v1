import { assertEquals, describe, Tx, types } from "./token-alex-src/deps.ts";
import { CoreClient } from "./token-alex-src/core-client.ts";
import { AuthClient } from "./token-alex-src/auth-client.ts";
import { TokenClient } from "./token-alex-src/token-client.ts";
...
//   });
  ...
  describe("TOKEN MANAGEMENT", () => {
    ...
    //     console.log(clients.auth.getCityWallet());
    //     const block = chain.mineBlock([
    //       clients.auth.setTokenUri(
    //         sender,
    //         clients.token.getContractAddress(),
    ...
    //     result.expectOk().expectUtf8(newUri);
    //   });
    });
  });
