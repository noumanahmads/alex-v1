...
(define-public (flash-loan-flat 
  ...
  (asserts! (is-ok (transfer-one flash-loan-user token1 amount1)) transfer-one-by-one-err)  
  (asserts! (is-ok (transfer-one flash-loan-user token2 amount2)) transfer-one-by-one-err)  
  (asserts! (is-ok (transfer-one flash-loan-user token3 amount3)) transfer-one-by-one-err)  
  ;; TODO: step 2 call user.execute
    (asserts! (is-ok (contract-call? flash-loan-user execute token1 token2 token3 amount1 amount2 amount3)) user-execute-err)
  ;; TODO: step 3 pay back tokens to vault
    (ok true)
  )
)
(define-private (transfer-one 
                  (flash-loan-user <flash-loan-user-trait-mod>) 
                  (token <ft-trait>)
                  (amount uint)
                  )
